...
# Last modified   : 2015-03-16 12:31:29
# Filename        : handler/weixin.py
# Description     : 
...
class WeiXinHandler(RealTimeHandler):
    ...
    def post(self):
        ...
        router = WeiXinRouter(result_data['Content'])
        _handler = route.get_handler()
        self.weixin = _handler(self, from_user = request_data['from_user'],
                to_user = request_data['to_user'], command = router.command,
                args = router.args, msg_type = msg_type) # 根据不同的事件，实例化不同的处理类，把自己传入，当服务器处理完后，把信息直接发给用户)
